...
class NewsController extends Controller
{
    ...
    public function index()
    {
        ...
        if(isset($_REQUEST["q"])){
                $news = News::where('category', 'like', '%cryptocurrency news%')->where('title', 'like', '%'.$_REQUEST["q"].'%')->orderByDesc('last_updated')->paginate($limit); 
        }
        else{
                $news = News::select('*')->where('category', 'like', '%cryptocurrency news%')->orderBy('last_updated', 'DESC')->paginate($limit);
         }
