...
final class CryptoCompare extends CryptoApiBase
{
    ...
    {
        $rows = $this->fetchJson(CryptoCompareHelper::build_url('events/'));

        foreach ($rows as &$item) {
            
            $item['pubDate'] = Carbon::createFromTimestampUTC($item['pubDate']);
            $item['tags'] = array_map(function ($s) {
                return trim($s);
            }, explode('|', strtolower($item['tags'])));
            $item['hashid'] = Murmur::hash3($item['guid']);
    ...
    {
        return $this->fetchJson(CryptoCompareHelper::build_url('event/providers'));
    }
}