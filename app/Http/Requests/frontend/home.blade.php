        ...
        <div class="col-12">
            ...
            <div class="ribbon-wrapper-reverse card">
                ...
                <div class="card-block">
                   <!--  <h2 class="card-title text-primary">{{ __('home.heading1') }}</h2> 
                     <p class="text-muted">{{ __('home.heading2', ['coin_count' => number_format($coins_approx) . '+']) }}</p>
                    <p class="card-text">
                        {{ __('home.description', ['app_name' => $app_name, 'coin_count' => number_format($coins_approx) . '+']) }}
                    </p> -->
                    <a href="{{ route('home.market') }}" class="btn btn-rounded btn-outline-success">{{ __('home.view_market') }}</a>
                </div>
            </div>
            ...
            <div class="card">
                <div class="row">
                    <div class="col-lg-4">
                        <div class="p-20">
                            <h4 class="card-subtitle">{{ __('home.highest_grossing') }}</h4>
                                    ...
                                    @foreach($top_coins as $symbol => $c)
                                        <a class="coin_list" href="#" data-attr="{{ $symbol }}">
                                            ...
                                            <div class="mail-contnet">
                                                <h5>{{ $c['name'] }} ({{ $symbol }})</h5>
                                                <span class="mail-desc">{{ __('coin.price') }}: {{ $c['price'] }}</span>
                                                <span class="time">{{ __('coin.change') }}: {{ $c['change'] }}%</span>
                                            </div>
                                        </a>
                                    @endforeach
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-block bg-light-success">
                    <div class="row">
                        <div class="col-6">
                            <h2 class="m-b-0">{{ __('home.top_gainers') }}</h2>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-block bg-light-warning">
                    <div class="row">
                        <div class="col-6">
                            <h2 class="m-b-0">{{ __('home.top_losers') }}</h2>
                        </div>
                    </div>
                </div>
                <div class="table-responsive">
