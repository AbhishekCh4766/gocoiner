<table class="table table-hover">
    ...
    @foreach($coins as $coin)
        <tr>
            <td>
                @if($coin->logo == '')
                <img src="{{ asset('asset/images/coins/tn/coin.png')}}" alt="Coin Image"/><a href="{{ coin_url($coin) }}"> {{ $coin->name }}</a>
                @else
                <img src="{{ asset('asset/images/coins/tn').'/'.$coin->logo }}" alt="Coin Image"/><a href="{{ coin_url($coin) }}"> {{ $coin->name }}</a>
                @endif
            </td>
